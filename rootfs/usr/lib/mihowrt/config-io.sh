...
yaml_prepend_autogenerated_scalars() {
	...
	awk -v socket="$socket" -v controller="$controller" -v secret="$secret" '
		function emit_patch() {
			if (patched)
				return
			print "# MihoWRT autogenerated API defaults"
			if (socket != "") print "external-controller-unix: " socket
			if (controller != "") print "external-controller: " controller
			if (secret != "") print "secret: " secret
			print ""
			patched = 1
		}
		!patched {
		...
		socket != "" && $0 ~ /^external-controller-unix:[[:space:]]*/ {
			next
		}
		controller != "" && $0 ~ /^external-controller:[[:space:]]*/ {
			next
		}
		secret != "" && $0 ~ /^secret:[[:space:]]*/ {
			next
		}
		{ print }
		END {
...
patch_config_api_defaults() {
	...
	if [ "$need_socket" -eq 1 ] || [ "$need_controller" -eq 1 ] || [ "$need_secret" -eq 1 ]; then
