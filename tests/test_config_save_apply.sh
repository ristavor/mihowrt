	...
	const context = {
		...
		backendHelper: {
			readServiceState: async() => {
				...
				return { available: true, serviceRunning: true, serviceEnabled: true, serviceReady: true, errors: [] };
			},
			...
			applyActiveConfig: async() => {
				context.disabledDuringApply = context.saveApplyButton.disabled;
				context.applyActiveConfigCalls += 1;
				return context.applyResult || { restartRequired: false, hotReloaded: true, policyReloaded: false };
			},
...
let saveConfigButton = globalThis.saveConfigButton;
...
async function readServiceState() {
	...
	return { running: !!status.serviceRunning, enabled: !!status.serviceEnabled, ready: !!status.serviceReady };
}
let pendingSubscriptionPersisted = false;
async function persistPendingSubscriptionSettings(configContent) {
	if (pendingSubscriptionPersisted)
		return false;
	...
	pendingSubscriptionPersisted = !!globalThis.pendingPersistResult;
	return pendingSubscriptionPersisted;
}
...
(async() => {
	...
	assert(restartFallback.appliedStates.length === 0, 'saveAndApply should leave service status rendering to the overview page');
	...
	assert(unchangedPending.serviceStatusCalls === 1, 'saveAndApply should read service state before applying the active configuration');
	assert(!unchangedPending.applyConfigPath && unchangedPending.applyActiveConfigCalls === 1, 'saveAndApply should apply the saved configuration when editor content is unchanged');
	...
	assert(unchangedPending.notifications.some(item => item.level === 'info' && String(item.message).includes('Configuration hot-reloaded.')), 'saveAndApply should report applying an unchanged saved configuration');
})().catch(err => {
	throw err;
});
EOF
