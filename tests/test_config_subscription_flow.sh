...
`, context);
	...
	assert(context.getPendingSubscriptionSettings() === null, 'persistPendingSubscriptionSettings should clear stale staged settings after content mismatch');
	assert(await context.persistPendingSubscriptionSettings('mode: rule\n') === false, 'persistPendingSubscriptionSettings should not resurrect stale settings after mismatch');
	context.stageSubscriptionSettings('https://example.com/sub.yaml', { content: result.content, profileUpdateInterval: '24', hotReloadSupported: true });
