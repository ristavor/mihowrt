...
eval "$(sed -n '/^runtime_policy_ready_state()/,/^}/p;/^load_snapshot_readiness_ports()/,/^}/p;/^load_config_readiness_ports()/,/^}/p;/^service_ready_runtime_state()/,/^}/p' "$ROOT_DIR/rootfs/usr/lib/mihowrt/runtime-status.sh")"
